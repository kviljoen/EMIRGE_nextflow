...
params.emirge_final = "$baseDir/emirge_results/iter.40"
 ...
 def helpMessage() {
    ...
    nextflow run kviljoen/EMIRGE_nextflow --reads '*_R{1,2}.fastq' --reference SILVA_138.1_SSURef_NR99_tax_silva_trunc.ge1200bp.le2000bp.0.97.fixed.fasta 
...
publishDir params.outdir, mode:'copy'
    ...
    def (read1, read2) = reads
      """
      emirge_amplicon.py ${params.outdir} -1 ${read1} -2 ${read2} -f ${params.reference} -b ${params.btindex} -i ${params.insert_size} -s {$params.insert_sd} -l ${params.max_len} --phred33 &> emirge_amplicon_std_out_err
      """
}
